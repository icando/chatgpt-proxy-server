...
go build -ldflags "-X 'github.com/icando/chatgpt-proxy-server/config.ENV=prod'" -o output/bin/${RUN_NAME}
echo -e "${BLUE_COLOR}编译完成${RES}\n"

echo -e "${YELOW_COLOR}---step3:杀掉进程并且运行---${RES}"
i1=`ps -ef|grep -E "xxx.yyy.chatgpt"|grep -v grep|awk '{print $2}'`
if [ -z $i1 ];then
...
i2=`ps -ef|grep -E "xxx.yyy.chatgpt"|grep -v grep|awk '{print $2}'`
